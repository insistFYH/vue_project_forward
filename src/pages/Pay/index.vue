...
export default {
    ...
    mounted() {
        ...
        this.getOrderPay(this.orderId);
    },
    methods: {
        //获取订单支付信息
        async getOrderPay(orderId) {
            let result = await this.$API.reqOrderPay(orderId);
            if (result.code == 200) {
        ...
        async open() {
            try {
                let result = await QRCode.toDataURL(this.orderPayInfo.codeUrl);
                // console.log(result);
                this.$alert(`<img src=${result} />`, "微信扫一扫", {
